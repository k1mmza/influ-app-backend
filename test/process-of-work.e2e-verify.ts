...
async function tokenFor(name: string, email: string, role: string) {
  ...
  if (!token)
    throw new Error(`No token for ${email}: ${JSON.stringify(login.json)}`);
...
async function main() {
  ...
  const brand = await tokenFor(
    'E2E Brand',
    `e2e.brand.${stamp}@influapp.test`,
    'BRAND',
  );
  const inf = await tokenFor(
    'E2E Influencer',
    `e2e.inf.${stamp}@influapp.test`,
    'INFLUENCER',
  );
  const outBrand = await tokenFor(
    'E2E OutBrand',
    `e2e.outbrand.${stamp}@influapp.test`,
    'BRAND',
  );
  const outInf = await tokenFor(
    'E2E OutInf',
    `e2e.outinf.${stamp}@influapp.test`,
    'INFLUENCER',
  );
  ...
  if (!infProfile)
    throw new Error('influencer profile missing after select-role');
  ...
  if (camp.status !== 201 && camp.status !== 200)
    throw new Error(
      `campaign create failed: ${camp.status} ${JSON.stringify(camp.json)}`,
    );
  ...
  if (!convId)
    throw new Error(`no conversation: ${JSON.stringify(accept.json)}`);
  console.log(
    `SETUP ok: campaignId=${campaignId} conversationId=${convId} influencerProfileId=${infProfile.id}\n`,
  );
  ...
  const brief1 = await http(
    'GET',
    `/conversations/${convId}/brief`,
    brand.token,
  );
  ...
  const p1 = await http(
    'POST',
    `/conversations/${convId}/phase-ready`,
    brand.token,
  );
  record(
    '2.1',
    ...
    p1.status === 200 || p1.status === 201
      ? p1.json.brandPhaseReady === true &&
          p1.json.influencerPhaseReady === false &&
          p1.json.workPhase === 'contact'
      : false,
  ...
  record(
    ...
    p2.json.workPhase === 'brief' &&
      p2.json.brandPhaseReady === false &&
      p2.json.influencerPhaseReady === false,
  ...
  // 2.4 negatives
  const negInf = await http(
    'POST',
    `/conversations/${convId}/phase-ready`,
    outInf.token,
  );
  const negBrand = await http(
    'POST',
    `/conversations/${convId}/phase-ready`,
    outBrand.token,
  );
  ...
  record(
    ...
    d1.status === 201 &&
      d1.json.status === 'DRAFT' &&
      d1.json.linkUrl?.includes('youtube'),
  ...
  const edit = await http(
    'PATCH',
    `/conversations/${convId}/drafts/${draftId}`,
    inf.token,
    {
      notes: 'edited notes',
    },
  );
  const dDel = await http(
    'POST',
    `/conversations/${convId}/drafts`,
    inf.token,
    { title: 'to delete' },
  );
  const del = await http(
    'DELETE',
    `/conversations/${convId}/drafts/${dDel.json.id}`,
    inf.token,
  );
  record(
    '4.2',
  ...
  const bCreate = await http(
    'POST',
    `/conversations/${convId}/drafts`,
    brand.token,
    { title: 'brand draft' },
  );
  const bEdit = await http(
    'PATCH',
    `/conversations/${convId}/drafts/${draftId}`,
    brand.token,
    { notes: 'x' },
  );
  const bDelete = await http(
    'DELETE',
    `/conversations/${convId}/drafts/${draftId}`,
    brand.token,
  );
  ...
  await http('PATCH', `/conversations/${convId}/drafts/${draftId}`, inf.token, {
    status: 'SUBMITTED',
  });
  const rev = await http(
    'PATCH',
    `/conversations/${convId}/drafts/${draftId}/review`,
    brand.token,
    {
  ...
  record(
    ...
    rev.json.status === 'REVISION_REQUESTED' &&
      revDraft?.revisionNote === 'Trim the intro and add captions',
  ...
  record(
    ...
    appr.json.status === 'APPROVED' &&
      (appr.json.revisionNote === null || appr.json.revisionNote === undefined),
  ...
  const fileResults: Record<
    string,
    { db: string; href: string; httpStatus: number; portable: boolean }
  > = {};
  async function checkUpload(
    ...
    token: string,
    fields: Record<string, string>,
    dbRead: () => Promise<string | null | undefined>,
  ) {
    ...
    fileResults[key] = {
      ...
      httpStatus: r.status,
      portable:
        String(dbVal).startsWith('/uploads/') &&
        !/^https?:\/\//i.test(String(dbVal)),
    };
    return up;
  }
  await checkUpload(
    ...
    { type: 'contract' },
    async () =>
      (await prisma.conversation.findUnique({ where: { id: convId } }))
        ?.contractUrl,
  );
  await checkUpload(
    'brief',
  ...
  await checkUpload(
    'draft',
    `/conversations/${convId}/drafts/${d2Id}/upload`,
    inf.token,
    {},
    async () =>
      (await prisma.draft.findUnique({ where: { id: d2Id } }))?.fileUrl,
  );
  // approve the file-only draft for Phase 5.2
  await http('PATCH', `/conversations/${convId}/drafts/${d2Id}`, inf.token, {
    status: 'SUBMITTED',
  });
  await http(
    'PATCH',
    `/conversations/${convId}/drafts/${d2Id}/review`,
    brand.token,
    { status: 'APPROVED' },
  );
  ...
  const payA = await http(
    'POST',
    `/conversations/${convId}/payments`,
    brand.token,
    { amount: 15000, paymentType: 'FIXED' },
  );
  record(
    '6.1',
    'Brand creates payment -> PENDING row written',
    payA.status === 201 &&
      payA.json.status === 'PENDING' &&
      payA.json.amount === 15000,
    `status=${payA.status} payment.status=${payA.json?.status} amount=${payA.json?.amount}`,
  );
  const payAId = payA.json.id;
  // 6.2 upload proof -> AWAITING_CONFIRMATION + file check
  const proofUp = await checkUpload(
    'payment',
    `/conversations/${convId}/payments/${payAId}/proof`,
    brand.token,
    {},
    async () =>
      (await prisma.payment.findUnique({ where: { id: payAId } }))?.proofUrl,
  );
  record(
    '6.2',
    'Brand uploads proof -> AWAITING_CONFIRMATION; proof opens (200)',
    proofUp.json?.status === 'AWAITING_CONFIRMATION' &&
      fileResults['payment'].httpStatus === 200,
    `payment.status=${proofUp.json?.status} proofHttp=${fileResults['payment'].httpStatus} href=${fileResults['payment'].href}`,
  );

  // 6.4 negatives BEFORE confirming A
  const payB = await http(
    'POST',
    `/conversations/${convId}/payments`,
    brand.token,
    { amount: 9000 },
  ); // PENDING
  const confPending = await http(
    'PATCH',
    `/conversations/${convId}/payments/${payB.json.id}/confirm`,
    inf.token,
  );
  const infCreate = await http(
    'POST',
    `/conversations/${convId}/payments`,
    inf.token,
    { amount: 100 },
  );
  const brandConfirm = await http(
    'PATCH',
    `/conversations/${convId}/payments/${payAId}/confirm`,
    brand.token,
  );
  ...
  record(
    '6.3',
    'Influencer confirms -> PAID, confirmedAt set, wallet credited once',
    conf.json?.status === 'PAID' && !!paymentRow?.confirmedAt && credited,
    `payment.status=${conf.json?.status} confirmedAt=${paymentRow?.confirmedAt?.toISOString?.() ?? paymentRow?.confirmedAt} wallet.totalEarned ${walletBefore?.totalEarned}->${walletAfter?.totalEarned} lastPaymentAmount=${walletAfter?.lastPaymentAmount}`,
  );
  const doubleConf = await http(
    'PATCH',
    `/conversations/${convId}/payments/${payAId}/confirm`,
    inf.token,
  );
  record(
    ...
    brandConfirm.status === 403 &&
      infCreate.status === 403 &&
      confPending.status === 400 &&
      doubleConf.status === 400,
  ...
  record(
    '6.x',
    'payments-update socket fired (both clients)',
    events['brandSock:payments-update'].length >= 1 &&
      events['infSock:payments-update'].length >= 1,
    `brandSock=${events['brandSock:payments-update'].length} infSock=${events['infSock:payments-update'].length}`,
  );
  ...
  for (const [k, f] of Object.entries(fileResults)) {
    record(
      `1.${k}`,
      `Upload [${k}]: DB portable + href resolves 200 on :3001`,
      f.portable && f.httpStatus === 200,
      `db=${f.db} href=${f.href} http=${f.httpStatus} portable=${f.portable}`,
    );
  }
  record(
    '1.summary',
    'All uploaded files: portable DB value + 200 via fileUrl()',
    allPortable && all200,
    `portable=${allPortable} all200=${all200}`,
  );

  // ============ PHASE 5 — PUBLISHED CONTENT ============
  const draftsNow = await http(
    'GET',
    `/conversations/${convId}/drafts`,
    inf.token,
  );
  const publishedLinks = draftsNow.json.filter(
    (d: any) => d.status === 'APPROVED' && !!d.linkUrl,
  );
  const approvedFileOnly = draftsNow.json.filter(
    (d: any) => d.status === 'APPROVED' && !d.linkUrl,
  );
  record(
    '5.1',
    ...
    publishedLinks.length === 1 &&
      publishedLinks[0].linkUrl === 'https://youtube.com/watch?v=e2e123',
  ...
  record(
    ...
    approvedFileOnly.length >= 1 &&
      approvedFileOnly.every((d: any) => !d.linkUrl) &&
      !publishedLinks.some((d: any) => !d.linkUrl),
  ...
  record(
    ...
    events['brandSock:phase-update'].length >= 2 &&
      events['infSock:phase-update'].length >= 2,
    ...
    '\nEVIDENCE_JSON ' +
      JSON.stringify({
        ...
        eventCounts: Object.fromEntries(
          Object.entries(events).map(([k, v]) => [k, v.length]),
        ),
      }),
  );
}

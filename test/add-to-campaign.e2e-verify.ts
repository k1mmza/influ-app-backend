...
async function tokenFor(name: string, email: string, role: string) {
  ...
  if (!token) throw new Error(`no token ${email}: ${JSON.stringify(login.json)}`);
...
async function main() {
  ...
  const brand = await tokenFor('ATC Brand', `atc.brand.${stamp}@influapp.test`, 'BRAND');
  const brand2 = await tokenFor('ATC Brand2', `atc.brand2.${stamp}@influapp.test`, 'BRAND');
  const uA = await tokenFor('ATC InfA', `atc.infa.${stamp}@influapp.test`, 'INFLUENCER');
  const uB = await tokenFor('ATC InfB', `atc.infb.${stamp}@influapp.test`, 'INFLUENCER');
  const uC = await tokenFor('ATC InfC', `atc.infc.${stamp}@influapp.test`, 'INFLUENCER');
  ...
  const c1 = await http('POST', '/campaigns', brand.token, { name: `ATC C1 ${stamp}`, visibility: 'PUBLIC' });
  const C1 = c1.json.id;
  const c2 = await http('POST', '/campaigns', brand2.token, { name: `ATC C2 ${stamp}`, visibility: 'PUBLIC' });
  const C2 = c2.json.id;

  // A1 — first invite
  const a1 = await http('POST', `/campaigns/${C1}/invite`, brand.token, { influencerId: infA });
  ...
  record(
    ...
    (a1.status === 200 || a1.status === 201) && a1.json.inviteResult === 'INVITED' &&
      rowA?.status === 'INVITED' && rowA?.origin === 'INVITATION',
    `status=${a1.status} inviteResult=${a1.json?.inviteResult} dbStatus=${rowA?.status} origin=${rowA?.origin}`,
  );

  // A2 — re-invite (idempotent)
  const a2 = await http('POST', `/campaigns/${C1}/invite`, brand.token, { influencerId: infA });
  const countA = await prisma.campaignApplication.count({ where: { campaignId: C1, influencerId: infA } });
  record(
    'A2',
  ...
  const a3 = await http('POST', `/campaigns/${C1}/invite`, brand.token, { influencerId: infB });
  record(
    'A3',
    'Influencer already applied (PENDING) -> invite returns 409 conflict',
    (applyB.status === 200 || applyB.status === 201) && a3.status === 409 &&
  ...
  const a4invite = await http('POST', `/campaigns/${C1}/invite`, brand.token, { influencerId: infC });
  const decline = await http('POST', `/invitations/${a4invite.json.id}/decline`, uC.token);
  const a4re = await http('POST', `/campaigns/${C1}/invite`, brand.token, { influencerId: infC });
  record(
    'A4',
    'Declined invite -> re-invite returns RE_INVITED (same row back to INVITED)',
    a4invite.json.inviteResult === 'INVITED' && (decline.status === 200 || decline.status === 201) &&
  ...
  const a5 = await http('POST', `/campaigns/${C1}/invite`, brand2.token, { influencerId: infA });
  record('A5', 'Brand that does NOT own the campaign -> 404', a5.status === 404, `status=${a5.status} msg=${JSON.stringify(a5.json?.message)}`);

  // A6 — non-existent influencer
  const a6 = await http('POST', `/campaigns/${C1}/invite`, brand.token, { influencerId: 'does-not-exist' });
  record('A6', 'Inviting a non-existent influencer -> 404', a6.status === 404, `status=${a6.status} msg=${JSON.stringify(a6.json?.message)}`);

  // A7 — role guard: influencer cannot invite
  const a7 = await http('POST', `/campaigns/${C1}/invite`, uA.token, { influencerId: infB });
  record('A7', 'Influencer role cannot invite -> 403', a7.status === 403, `status=${a7.status}`);
  ...
  const myInvite = (invites.json ?? []).find((i: any) => i.campaignId === C1) ?? { id: a1.json.id };
  const accept = await http('POST', `/invitations/${myInvite.id}/accept`, uA.token);
  const conv = await prisma.conversation.findFirst({ where: { campaignId: C1, influencerId: infA } });
  const a8re = await http('POST', `/campaigns/${C1}/invite`, brand.token, { influencerId: infA });
  record(
    'A8',
    'Invited influencer accepts -> conversation created; re-invite -> ALREADY_ACCEPTED',
    (accept.status === 200 || accept.status === 201) && !!conv && a8re.json.inviteResult === 'ALREADY_ACCEPTED',

...
async function main() {
  ...
  const brand = await tokenFor('CM Brand', `cm.brand.${stamp}@influapp.test`, 'BRAND');
  const brand2 = await tokenFor('CM Brand2', `cm.brand2.${stamp}@influapp.test`, 'BRAND');
  const inf = await tokenFor('CM Inf', `cm.inf.${stamp}@influapp.test`, 'INFLUENCER');
  ...
  record(
    ...
    (create.status === 200 || create.status === 201) && create.json?.status === 'DRAFT' && !!create.json?.clientBrandId,
  ...
  record(
    ...
    Array.isArray(list.json) && list.json.some((c: any) => c.id === CA) &&
      Array.isArray(listInf.json) && listInf.json.length === 0,
  ...
  record(
    ...
    detailOwner.status === 200 && 'requirements' in detailOwner.json && 'applications' in detailOwner.json &&
      detailOther.status === 404,
    `owner=${detailOwner.status} nonOwner=${detailOther.status}`,
  );

  // CM4 — update fields + ownership guard
  const upd = await http('PATCH', `/campaigns/${CA}`, brand.token, { name: `CM Renamed ${stamp}`, budget: 35000 });
  const updOther = await http('PATCH', `/campaigns/${CA}`, brand2.token, { name: 'hijack' });
  record(
    'CM4',
    'Update persists owner edits; non-owner update -> 404',
    upd.status === 200 && upd.json?.name === `CM Renamed ${stamp}` && upd.json?.budget === 35000 && updOther.status === 404,
  ...
  const toActive = await http('PATCH', `/campaigns/${CA}`, brand.token, { status: 'ACTIVE' });
  const backToDraft = await http('PATCH', `/campaigns/${CA}`, brand.token, { status: 'DRAFT' });
  record(
    'CM5',
    'Status: DRAFT->ACTIVE allowed; ACTIVE->DRAFT rejected (400 invalid transition)',
    toActive.status === 200 && toActive.json?.status === 'ACTIVE' && backToDraft.status === 400,
  ...
  await http('PATCH', `/campaigns/${CA}`, brand.token, { visibility: 'PUBLIC' });
  const draftOnly = await http('POST', '/campaigns', brand.token, { name: `CM Draft ${stamp}`, visibility: 'PUBLIC' }); // stays DRAFT
  ...
  record(
    ...
    publicList.status === 200 && ids.includes(CA) && !ids.includes(draftOnly.json?.id),
  ...
  const appsBefore = await http('GET', `/campaigns/${CA}/applications`, brand.token);
  const appRow = (appsBefore.json ?? [])[0];
  const beforeConv = appRow?.conversationId ?? null;
  const accept = await http('PATCH', `/campaigns/${CA}/applications/${appRow?.id}`, brand.token, { status: 'ACCEPTED' });
  const appsAfter = await http('GET', `/campaigns/${CA}/applications`, brand.token);
  const afterConv = (appsAfter.json ?? []).find((a: any) => a.id === appRow?.id)?.conversationId;
  record(
    'CM7',
    'Applications: applicant listed (conversationId null), accept creates conversation + populates conversationId',
    (apply.status === 200 || apply.status === 201) && !!appRow && beforeConv === null &&
      accept.json?.conversationId && afterConv === accept.json.conversationId,
  ...
  const convGone = await prisma.conversation.findFirst({ where: { campaignId: CA } });
  const appsGone = await prisma.campaignApplication.findFirst({ where: { campaignId: CA } });
  record(
    'CM8b',
    'Delete CANCELLED campaign -> cascade removes campaign + conversation + applications',
    (delCancelled.status === 200 || delCancelled.status === 204) && !campGone && !convGone && !appsGone,
  ...
  const noName = await http('POST', '/campaigns', brand.token, { objective: 'x' });
  const infCreate = await http('POST', '/campaigns', inf.token, { name: 'inf camp' });
  record(
    'CM9',

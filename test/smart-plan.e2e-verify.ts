...
async function main() {
  ...
  const brand = await tokenFor(
    'SP Brand',
    `sp.brand.${stamp}@influapp.test`,
    'BRAND',
  );
  const inf = await tokenFor(
    'SP Inf',
    `sp.inf.${stamp}@influapp.test`,
    'INFLUENCER',
  );

  // ── Role guards: INFLUENCER forbidden on every smart-plan route ──
  const gInf = await http('POST', '/smart-plan/generate', inf.token, {
    campaignName: 'x',
  });
  const sInf = await http('POST', '/smart-plan/save', inf.token, {
    strategy: 'x',
  });
  const bInf = await http('GET', '/smart-plan/brief', inf.token);
  const cInf = await http('POST', '/smart-plan/create-campaign', inf.token, {
    campaignFields: { name: 'x' },
  });
  record(
    'G1',
  ...
  const badCreate = await http(
    'POST',
    '/smart-plan/create-campaign',
    brand.token,
    { strategy: 'no fields' },
  );
  record(
  ...
  const shapeOk =
    ...
    typeof fg.strategy === 'string' &&
    fg.strategy.length > 0 &&
    typeof fg.concept === 'string' &&
    fg.concept.length > 0 &&
    typeof fg.briefBody === 'string' &&
    fg.briefBody.length > 0 &&
  ...
  record(
    ...
    Array.isArray(fg.provenance?.userProvided) &&
      fg.provenance.userProvided.includes('name'),
  ...
  const promptGen = await http('POST', '/smart-plan/generate', brand.token, {
    rawPrompt:
      'Launch a budget-friendly iced-coffee brand to Gen Z in Bangkok via TikTok creators.',
  });
  ...
  record(
    ...
    (promptGen.status === 200 || promptGen.status === 201) &&
      typeof pg.strategy === 'string' &&
      pg.strategy.length > 0 &&
      typeof pg.campaignFields?.name === 'string' &&
      pg.campaignFields.name.length > 0 &&
      Array.isArray(pg.provenance?.userProvided) &&
      pg.provenance.userProvided.length === 0,
  ...
  record(
    ...
    (saveStandalone.status === 200 || saveStandalone.status === 201) &&
      restore.json?.strategy === strat,
  ...
  const createFromPlan = await http(
    'POST',
    '/smart-plan/create-campaign',
    brand.token,
    {
  ...
  const briefRow = newCampaignId
    ? await prisma.smartPlanBrief.findFirst({
  ...
  record(
    ...
    (createFromPlan.status === 200 || createFromPlan.status === 201) &&
      ...
      campaignRow.keyMessage === 'Glow all summer' &&
      campaignRow.budget === 30000 &&
      campaignRow.requirements.length === 1 &&
      campaignRow.requirements[0].minFollowers === 5000,
  ...
  record(
    ...
    !!briefRow &&
      briefRow.strategy === 'plan-strategy' &&
      briefRow.inputMode === 'AI' &&
      briefRow.campaignId === newCampaignId,
  ...
  const byCampaign = await http(
    'GET',
    `/smart-plan/brief/by-campaign/${newCampaignId}`,
    brand.token,
  );
  record(
    'BYCAMP1',
    'GET /brief/by-campaign/:id returns the attached brief',
    byCampaign.status === 200 &&
      byCampaign.json?.strategy === 'plan-strategy' &&
      byCampaign.json?.briefBody === 'plan-brief',
  ...
  await http('POST', '/smart-plan/save', brand.token, {
    strategy: `S2-${stamp}`,
  });
  const briefStillThere = await prisma.smartPlanBrief.findFirst({
    where: { campaignId: newCampaignId },
  });
  const standaloneCount = await prisma.smartPlanBrief.count({
    where: { createdBy: brand.userId, campaignId: null },
  });
  record(
    'SCOPE1',

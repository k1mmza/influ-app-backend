...
describe('ProfileController', () => {
  ...
  describe('GET /profile', () => {
    it('200 returns profile for authenticated user', async () => {
      profileSvc.getProfile.mockResolvedValueOnce({ id: TEST_USER_ID, name: 'Test User' });
  ...
  describe('GET /profile/completeness', () => {
    it('200 returns completeness score', async () => {
      profileSvc.getCompleteness.mockResolvedValueOnce({ score: 75, missing: ['bio'] });
      const res = await request(app.getHttpServer()).get('/profile/completeness').expect(200);
  ...
  describe('PATCH /profile', () => {
    it('200 updates profile with partial fields', async () => {
      profileSvc.updateProfile.mockResolvedValueOnce({ id: TEST_USER_ID, name: 'New Name' });
      await request(app.getHttpServer())
        .patch('/profile')
        .send({ name: 'New Name' })
        .expect(200);
      expect(profileSvc.updateProfile).toHaveBeenCalledWith(TEST_USER_ID, expect.objectContaining({ name: 'New Name' }));
    });
  ...
  describe('POST /profile/media-kit/analyze', () => {
    it('201 analyzes uploaded media kit file', async () => {
      mediaKitSvc.analyzeFile.mockResolvedValueOnce({ followers: 10000, engagementRate: 3.2 });
      const res = await request(app.getHttpServer())
        .post('/profile/media-kit/analyze')
        .attach('file', Buffer.from('{}'), { filename: 'media-kit.json', contentType: 'application/json' })
        .expect(201);
    ...
    it('403 with wrong role', async () => {
      ...
      await request(noRoleApp.getHttpServer())
        .post('/profile/media-kit/analyze')
        .attach('file', Buffer.from('{}'), { filename: 'kit.json', contentType: 'application/json' })
        .expect(403);
  ...
  describe('POST /profile/avatar', () => {
    it('201 uploads avatar and returns file URL', async () => {
      profileSvc.uploadAvatarFile.mockResolvedValueOnce({ avatarUrl: '/uploads/avatars/test.jpg' });
      const res = await request(app.getHttpServer())
        .post('/profile/avatar')
        .attach('file', Buffer.from('fake-image'), { filename: 'avatar.jpg', contentType: 'image/jpeg' })
        .expect(201);
  ...
  describe('POST /profile/rate-card', () => {
    it('201 uploads rate card', async () => {
      profileSvc.uploadRateCardFile.mockResolvedValueOnce({ rateCardUrl: '/uploads/rate-cards/card.pdf' });
      await request(app.getHttpServer())
        .post('/profile/rate-card')
        .attach('file', Buffer.from('pdf-content'), { filename: 'rate-card.pdf', contentType: 'application/pdf' })
        .expect(201);
  ...
  describe('DELETE /profile/rate-card', () => {
    it('200 deletes rate card', async () => {
      profileSvc.deleteRateCardFile.mockResolvedValueOnce({ message: 'deleted' });
      await request(app.getHttpServer()).delete('/profile/rate-card').expect(200);

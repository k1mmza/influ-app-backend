...
describe('ConversationsController', () => {
  ...
  describe('POST /conversations', () => {
    it('201 creates or returns existing conversation', async () => {
      ...
      expect(svc.createOrFind).toHaveBeenCalledWith(TEST_USER_ID, 'inf-1', 'camp-1');
    });
  });
  ...
  describe('GET /conversations/:id', () => {
    it('200 returns single conversation', async () => {
      ...
      const res = await request(app.getHttpServer()).get('/conversations/conv-1').expect(200);
      expect(res.body.id).toBe('conv-1');
    });

    it('404 when conversation not found', async () => {
      svc.findOne.mockRejectedValueOnce(new NotFoundException());
      await request(app.getHttpServer()).get('/conversations/nonexistent').expect(404);
    });
  });

  describe('GET /conversations/:id/messages', () => {
    it('200 returns messages for conversation', async () => {
      svc.findMessages.mockResolvedValueOnce([{ id: 'msg-1', content: 'Hello' }]);
      const res = await request(app.getHttpServer()).get('/conversations/conv-1/messages').expect(200);
  ...
  describe('POST /conversations/:id/messages', () => {
    it('201 sends a message', async () => {
      ...
      await request(app.getHttpServer())
        ...
        .send({ content: 'Hello' })
        .expect(201);
      expect(svc.sendMessage).toHaveBeenCalledWith(TEST_USER_ID, 'conv-1', 'Hello');
    });
  });

  describe('PATCH /conversations/:id/phase', () => {
    it('200 updates work phase', async () => {
      svc.updatePhase.mockResolvedValueOnce({ id: 'conv-1', workPhase: 'PRODUCTION' });
      await request(app.getHttpServer())
  ...
  describe('PATCH /conversations/:id/read', () => {
    it('200 marks conversation as read', async () => {
      ...
      await request(app.getHttpServer()).patch('/conversations/conv-1/read').expect(200);
  ...
  describe('POST /conversations/:id/phase-ready', () => {
    it('201 marks phase as ready', async () => {
      ...
      await request(app.getHttpServer()).post('/conversations/conv-1/phase-ready').expect(201);
  ...
  describe('GET /conversations/:id/brief', () => {
    it('200 returns brief for conversation', async () => {
      ...
      const res = await request(app.getHttpServer()).get('/conversations/conv-1/brief').expect(200);
  ...
  describe('POST /conversations/:id/upload', () => {
    it('201 uploads file and saves attachment', async () => {
      svc.saveAttachment.mockResolvedValueOnce({ url: '/uploads/conversations/test.jpg' });
      await request(app.getHttpServer())
        .post('/conversations/conv-1/upload')
        .attach('file', Buffer.from('fake-image-data'), { filename: 'test.jpg', contentType: 'image/jpeg' })
        .field('type', 'image')
        .expect(201);
      expect(svc.saveAttachment).toHaveBeenCalledWith('conv-1', 'image', expect.stringContaining('/uploads/conversations/'));
    });
  });
  ...
  describe('Auth failures', () => {
    ...
    it('401 GET /conversations without token', async () => {
      await request(noAuthApp.getHttpServer()).get('/conversations').expect(401);
    });

    it('401 POST /conversations without token', async () => {
      await request(noAuthApp.getHttpServer()).post('/conversations').send({}).expect(401);
    });
  });
});

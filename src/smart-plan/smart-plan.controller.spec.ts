...
describe('SmartPlanController', () => {
  ...
  describe('POST /smart-plan/generate', () => {
    it('201 generates brief from structured fields', async () => {
      svc.generate.mockResolvedValueOnce({ strategy: 'Focus on...', concept: 'Create...' });
      ...
      expect(svc.generate).toHaveBeenCalledWith(expect.objectContaining({ campaignName: 'Summer Launch' }));
    });

    it('201 with empty body (all fields optional)', async () => {
      svc.generate.mockResolvedValueOnce({ strategy: 'Default strategy' });
      await request(app.getHttpServer()).post('/smart-plan/generate').send({}).expect(201);
    });
    ...
    it('201 with rawPrompt only', async () => {
      ...
      await request(app.getHttpServer())
        .post('/smart-plan/generate')
        .send({ rawPrompt: 'Create a campaign for a new sneaker brand targeting Gen Z' })
        .expect(201);
    });
  ...
  describe('POST /smart-plan/create-campaign', () => {
    it('201 creates campaign from plan', async () => {
      svc.createCampaignFromPlan.mockResolvedValueOnce({ id: 'camp-1', name: 'New Campaign' });
      const res = await request(app.getHttpServer())
      ...
      expect(svc.createCampaignFromPlan).toHaveBeenCalledWith(
        TEST_USER_ID,
        expect.objectContaining({ campaignFields: { name: 'New Campaign', budget: 5000 } }),
      );
    });
  ...
  describe('POST /smart-plan/save', () => {
    it('201 saves brief for user', async () => {
      ...
      await request(app.getHttpServer())
        ...
        .send({ strategy: 'Focus on engagement', briefBody: 'Full brief text...' })
        .expect(201);
      expect(svc.saveBrief).toHaveBeenCalledWith(TEST_USER_ID, expect.objectContaining({ strategy: 'Focus on engagement' }));
    });

    it('201 with empty body (all fields optional)', async () => {
      svc.saveBrief.mockResolvedValueOnce({ id: 'brief-1' });
      await request(app.getHttpServer()).post('/smart-plan/save').send({}).expect(201);
    });
  });

  describe('GET /smart-plan/brief', () => {
    it('200 returns latest standalone brief', async () => {
      svc.getLatestBrief.mockResolvedValueOnce({ id: 'brief-1', strategy: 'Growth strategy' });
      const res = await request(app.getHttpServer()).get('/smart-plan/brief').expect(200);
  ...
  describe('GET /smart-plan/brief/by-campaign/:campaignId', () => {
    it('200 returns brief for campaign', async () => {
      svc.getBriefByCampaign.mockResolvedValueOnce({ id: 'brief-1', campaignId: 'camp-1' });
      const res = await request(app.getHttpServer()).get('/smart-plan/brief/by-campaign/camp-1').expect(200);
    ...
    it('404 when brief not found for campaign', async () => {
      ...
      await request(app.getHttpServer()).get('/smart-plan/brief/by-campaign/nonexistent').expect(404);
    });
  });
  ...
  describe('Auth and role failures', () => {
    it('401 POST /smart-plan/generate without token', async () => {
      ...
      await request(noAuthApp.getHttpServer()).post('/smart-plan/generate').send({}).expect(401);
      await noAuthApp.close();
    });

    it('403 GET /smart-plan/brief with wrong role', async () => {
      const noRoleApp = await buildApp(AUTH_PASS, ROLE_FAIL);
      await request(noRoleApp.getHttpServer()).get('/smart-plan/brief').expect(403);

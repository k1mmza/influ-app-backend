...
describe('CampaignsController', () => {
  ...
  describe('GET /campaigns', () => {
    it('200 returns campaigns for the authenticated user', async () => {
      svc.getCampaignsForUser.mockResolvedValueOnce([{ id: 'c1', name: 'Camp 1' }]);
  ...
  describe('POST /campaigns', () => {
    it('201 creates campaign with required name', async () => {
      svc.createCampaign.mockResolvedValueOnce({ id: 'c1', name: 'My Campaign' });
      const res = await request(app.getHttpServer())
        .post('/campaigns')
        .send({ name: 'My Campaign' })
        .expect(201);
      expect(res.body.name).toBe('My Campaign');
      expect(svc.createCampaign).toHaveBeenCalledWith(TEST_USER_ID, expect.objectContaining({ name: 'My Campaign' }));
    });

    it('400 on missing name', async () => {
      await request(app.getHttpServer()).post('/campaigns').send({}).expect(400);
    });
  ...
  describe('GET /campaigns/public', () => {
    ...
    it('200 with custom page and pageSize', async () => {
      svc.getPublicCampaigns.mockResolvedValueOnce({ items: [], total: 0 });
      await request(app.getHttpServer()).get('/campaigns/public?page=2&pageSize=5').expect(200);
  ...
  describe('GET /campaigns/:id', () => {
    it('200 returns campaign by id', async () => {
      svc.getCampaign.mockResolvedValueOnce({ id: 'c1', name: 'Camp', isDeleted: false });
      const res = await request(app.getHttpServer()).get('/campaigns/c1').expect(200);
      expect(res.body.id).toBe('c1');
    });

    it('404 when campaign not found', async () => {
      svc.getCampaign.mockRejectedValueOnce(new NotFoundException('Campaign not found'));
      await request(app.getHttpServer()).get('/campaigns/nonexistent').expect(404);
    });
  });
  ...
  describe('PATCH /campaigns/:id', () => {
    it('200 updates campaign fields', async () => {
      ...
      await request(app.getHttpServer()).patch('/campaigns/c1').send({ name: 'Updated' }).expect(200);
      expect(svc.updateCampaign).toHaveBeenCalledWith(TEST_USER_ID, 'c1', expect.objectContaining({ name: 'Updated' }));
    });
    ...
    it('200 with valid status ACTIVE', async () => {
      ...
      await request(app.getHttpServer()).patch('/campaigns/c1').send({ status: 'ACTIVE' }).expect(200);
    });

    it('404 when campaign not found', async () => {
      svc.updateCampaign.mockRejectedValueOnce(new NotFoundException());
      await request(app.getHttpServer()).patch('/campaigns/nonexistent').send({ name: 'X' }).expect(404);
    });
  });
  ...
  describe('DELETE /campaigns/:id', () => {
    ...
    it('404 when campaign not found', async () => {
      ...
      await request(app.getHttpServer()).delete('/campaigns/nonexistent').expect(404);
    });
  });

  describe('POST /campaigns/:id/apply', () => {
    it('201 applies to campaign', async () => {
      svc.applyToCampaign.mockResolvedValueOnce({ id: 'app-1', status: 'PENDING' });
      await request(app.getHttpServer()).post('/campaigns/c1/apply').expect(201);
    ...
    it('404 when campaign not found', async () => {
      ...
      await request(app.getHttpServer()).post('/campaigns/nonexistent/apply').expect(404);
    });
  });

  describe('GET /campaigns/:id/applications', () => {
    it('200 returns list of applications', async () => {
      svc.getApplications.mockResolvedValueOnce([{ id: 'app-1', status: 'PENDING' }]);
      const res = await request(app.getHttpServer()).get('/campaigns/c1/applications').expect(200);
  ...
  describe('PATCH /campaigns/:id/applications/:applicationId', () => {
    it('200 updates application status', async () => {
      svc.updateApplicationStatus.mockResolvedValueOnce({ id: 'app-1', status: 'APPROVED' });
      await request(app.getHttpServer())
        .patch('/campaigns/c1/applications/app-1')
        .send({ status: 'APPROVED' })
        .expect(200);
      expect(svc.updateApplicationStatus).toHaveBeenCalledWith(TEST_USER_ID, 'c1', 'app-1', 'APPROVED');
    });
  });
  ...
  describe('Auth failures', () => {
    ...
    beforeAll(async () => { noAuthApp = await buildApp(AUTH_FAIL); });
    ...
    it('401 POST /campaigns without token', async () => {
      await request(noAuthApp.getHttpServer()).post('/campaigns').send({ name: 'X' }).expect(401);
    });
  });
});
